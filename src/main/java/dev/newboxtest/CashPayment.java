...
public class CashPayment {
    ...
    public CashPayment(Config config, ScreenshotUtil screenshotUtil, ExcelUtils excelUtil) {
        this.config = config;
    ...
    public void loginAsMaster(WebDriver driver) throws Exception {
        try {
            ...
            for (String[] row : excelData) {
                ...
                performCashPaymentActions(driver);
...
//                login(driver, config.getMasterUsername(), config.getPassword());
    ...
    private void performCashPaymentActions(WebDriver driver) throws Exception {
        try {
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));

    	   // Locate and click the PaidIconn SVG element
        WebElement PaidIcon = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("[data-testid='PaidIcon']")));
        PaidIcon.click();
...
//        WebElement EastIcon7 = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("[data-testid='EastIcon']")));
//        EastIcon7.click();
        
       
        WebElement doneicon4 = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("/html/body/div[1]/div[1]/div[2]/div/div[3]/button[1]")));
        doneicon4.click();
        
        WebElement okButton2 = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button.MuiButtonBase-root.MuiButton-root.MuiButton-containedPrimary")));
        System.out.println("Clicking OK button");
        okButton2.click();
...
//    }
   ...
   public static void main(String[] args) {
        ...
        try {
            ...
            CashPayment cashPayment = new CashPayment(config, screenshotUtil, excelUtil);
            cashPayment.loginAsMaster(driver);
